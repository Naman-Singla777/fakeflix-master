...
const MovieDetails = ({
  ...
  const fetchsimiliarmovies = async () => {
    ...
    axios({
      method: "post",
      url: "https://fakeflix-backend-01.herokuapp.com/recommend_movie",
      data: formdata,
...
const fetchtvseries =  async (media_type) => {
     const { data } = await axios.get(
       `https://api.themoviedb.org/3/${media_type.toLowerCase()}/${id}/similar?api_key=dd81438b8de670df53d6e2e3c5f115ed&language=en-US`
     );
